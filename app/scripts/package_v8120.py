#!/usr/bin/env python3
"""Package Nova v8.12.0 "master switch" - verify then zip (same
...
PREV_CANDIDATES = [
    _OUT_DIR / "nova_assistant_v8.11.0_fixed.zip",
    Path(__file__).resolve().parents[2] / ".." / "download"
    / "nova_assistant_v8.11.0_fixed.zip",
]
...
probe("nova.VERSION == 8.12.0", nova.VERSION == "8.12.0", nova.VERSION)
probe("nova_think.VERSION == 8.12.0", think.VERSION == "8.12.0",
      think.VERSION)
probe("codename is master switch",
      nova.CODENAME == "master switch", nova.CODENAME)
stray = []
for p in (APP / "tests").glob("test_*.py"):
    raw = p.read_text(encoding="utf-8", errors="replace")
    if 'VERSION, "8.11.0"' in raw:
        stray.append(p.name)
probe("no stray 8.11.0 VERSION pins in tests", not stray, stray)
...
print("[probe 3] release sanity (v8.12.0 + v8.11.0 laws)")
...
# --- v8.12 laws: the master switches are absolute + input honesty ---
# M1: apply_approved honors the guardian master switch (env kill-switch)
import time as _now
os.environ["NOVA_GUARDIAN"] = "0"
try:
    dw1 = tempfile.mkdtemp()
    s1 = nova.Session(Path(dw1))
    s1.confirm_changes = True
    nova.NONINTERACTIVE = True
    try:
        _f, _u = nova.parse_files("=== FILE: bad.go ===\n" +
                                  'package main\nfunc main() {\n' +
                                  'println("x")\n=== END ===\n')
        nova.offer_apply(s1, _f, _u, auto=True)
        pend1 = s1.pending_apply
        probe("M1: NONINTERACTIVE stages the batch", bool(pend1))
        _calls = {"n": 0}
        _orig = nova.guardian.review_batch

        def _spy(*a, **k):
            _calls["n"] += 1
            return _orig(*a, **k)
        nova.guardian.review_batch = _spy
        try:
            ok1, r1 = nova.apply_approved(s1, pend1["id"], ["bad.go"], {})
        finally:
            nova.guardian.review_batch = _orig
        probe("M1: guardian OFF - web Approve applies, fleet never runs",
              ok1 and _calls["n"] == 0 and (Path(dw1) / "bad.go").exists(),
              (ok1, _calls["n"]))
        # M2: the draft gate never votes while OFF
        _calls["n"] = 0
        nova._draft_valid("=== FILE: a.js ===\nvar x = 1;\n"
                          "Run: node a.js", s1)
        probe("M2: draft gate skips the vote while OFF", _calls["n"] == 0)
    finally:
        nova.NONINTERACTIVE = False
        import shutil as _sh
        _sh.rmtree(dw1, ignore_errors=True)
finally:
    del os.environ["NOVA_GUARDIAN"]
# M2b: with the fleet ON the vote still happens
_calls2 = {"n": 0}
_orig2 = nova.guardian.review_batch

def _spy2(*a, **k):
    _calls2["n"] += 1
    return _orig2(*a, **k)
nova.guardian.review_batch = _spy2
try:
    nova._draft_valid("=== FILE: a.js ===\nvar x = 1;\nRun: node a.js",
                      None)
finally:
    nova.guardian.review_batch = _orig2
probe("M2b: fleet ON still votes in the draft gate", _calls2["n"] == 1)

# M3: deep-on / wiring-off still fires the pre-apply gate
_dw3 = tempfile.mkdtemp()
_s3 = nova.Session(Path(_dw3))
_s3.probe_settings = {"on": True, "wiring": False, "deep": True,
                      "reject_wiring": True, "reject_deep": True}
_gate = {"n": 0}
_gorig = nova_probe.pre_apply_gate

def _gspy(*a, **k):
    _gate["n"] += 1
    return _gorig(*a, **k)
nova_probe.pre_apply_gate = _gspy
try:
    nova.NONINTERACTIVE = False
    _f3, _u3 = nova.parse_files("=== FILE: app.py ===\n"
                                "print(undefined_name)\n=== END ===\n")
    nova.offer_apply(_s3, _f3, _u3, auto=True)
finally:
    nova_probe.pre_apply_gate = _gorig
probe("M3: deep-on/wiring-off still calls the pre-apply gate",
      _gate["n"] == 1)
import shutil as _sh2
_sh2.rmtree(_dw3, ignore_errors=True)
# M3b: inside the gate the wiring toggle really controls wiring_check
_wc = {"n": 0}
_worig = nova_probe.wiring_check

def _wspy(*a, **k):
    _wc["n"] += 1
    return _worig(*a, **k)
nova_probe.wiring_check = _wspy
try:
    rep3 = nova_probe.pre_apply_gate(
        [("app.py", "print(undefined_name)")],
        cfg={"on": True, "wiring": False, "deep": True,
             "reject_wiring": True, "reject_deep": True})
finally:
    nova_probe.wiring_check = _worig
probe("M3b: wiring off = wiring probe skipped, deep still rejects",
      _wc["n"] == 0 and rep3["reject"])

# M4: probe.json garbage never claims custom
_dw4 = tempfile.mkdtemp()
(Path(_dw4) / ".nova").mkdir()
(Path(_dw4) / ".nova" / "probe.json").write_text(
    '{"junk": 1, "on": "false"}', encoding="utf-8")
probe("M4: zero-valid-keys probe.json does not claim custom",
      nova_probe.load_settings(_dw4)["custom"] is False)
_sh2.rmtree(_dw4, ignore_errors=True)

# M5: the preview hijack is gone (generator commands run)
probe("M5: python gen.py out.html is NOT a preview",
      nova.preview_target(Path(tempfile.mkdtemp()),
                          "python gen.py out.html") is None)
probe("M5b: node gen.js page.html is NOT a preview",
      nova.preview_target(Path(tempfile.mkdtemp()),
                          "node gen.js page.html") is None)
probe("M5c: a bare document is STILL a preview",
      nova.preview_target(Path(tempfile.mkdtemp()),
                          "index.html") is not None)
probe("M5d: cmd /c start is STILL a preview",
      nova.preview_target(Path(tempfile.mkdtemp()),
                          "cmd /c start index.html") is not None)
probe("M5e: a document named node.js.html is STILL a preview",
      nova.preview_target(Path(tempfile.mkdtemp()),
                          "node.js.html") is not None)

# M6: parse_layers never raises; Persian digits still parse
try:
    nova_design.parse_layers("shadow=\u00b2")
    _raised = False
except ValueError:
    _raised = True
probe("M6: superscript digit no longer raises", not _raised)
probe("M6b: Persian digits still parse",
      nova_design.parse_layers("shadow=\u06f2")["shadow"] == 2)

# M7: the workspace walk prunes .git / node_modules DURING the walk
_dw7 = tempfile.mkdtemp()
for _junk in (".git", "node_modules"):
    (Path(_dw7) / _junk).mkdir()
    for _i in range(400):
        (Path(_dw7) / _junk / ("junk%d.js" % _i)).write_text("var a=1;\n",
                                                            encoding="utf-8")
(Path(_dw7) / "app.js").write_text("var a = ;\n", encoding="utf-8")
_res7 = gd.audit_workspace(_dw7)
probe("M7: the audit only sees the real file (pruned walk)",
      [r["file"] for r in _res7] == ["app.js"],
      [r["file"] for r in _res7])
_sh2.rmtree(_dw7, ignore_errors=True)

# M8: nan/inf env timeouts fall back to the default
os.environ["NOVA_PROBE_TIMEOUT"] = "nan"
_v = nova_probe._env_float("NOVA_PROBE_TIMEOUT", 8.0)
del os.environ["NOVA_PROBE_TIMEOUT"]
probe("M8: nan env timeout falls back to the default", _v == 8.0, _v)

# M9-M11: /api/settings honesty (source-law probes; behavior is covered
# by test_v8120_master_switch.py driving the REAL server)
_ws_src = (APP / "web_server.py").read_text(encoding="utf-8")
probe("M9: reset keys demand a real boolean",
      'rkey + " must be a boolean"' in _ws_src
      and '"ctx_reset", "guardian_reset", "probe_reset"' in _ws_src
      and '"vision_reset"' in _ws_src)
probe("M10: session toggles reject non-bool honestly",
      'key + " must be a boolean"' in _ws_src
      and '"confirm_changes", "explain", "autotest", "autofix"' in _ws_src)
probe("M11: a missing engine layer answers 501",
      "the context-engine layer is unavailable" in _ws_src
      and "the code-guardian layer is unavailable" in _ws_src
      and "the bug-hunter layer is unavailable" in _ws_src
      and "the vision layer is unavailable" in _ws_src)
probe("M11b: the four dead result boxes are gone from the page",
      "ctxResult" not in (APP / "web" / "index.html").read_text(
          encoding="utf-8"))

# M12: the loop-guard flag starts fresh on every offer_apply
_dw12 = tempfile.mkdtemp()
_s12 = nova.Session(Path(_dw12))
_s12.batch_refused = True
_f12, _u12 = nova.parse_files("=== FILE: ok.js ===\nvar a = 1;\n"
                              "=== END ===\n")
nova.NONINTERACTIVE = False
_s12.confirm_changes = False
with open(os.devnull, "w") as _dn, \
        __import__("contextlib").redirect_stdout(_dn):
    nova.offer_apply(_s12, _f12, _u12, auto=True)
probe("M12: a successful apply clears a stale refusal flag",
      not getattr(_s12, "batch_refused", None))
_sh2.rmtree(_dw12, ignore_errors=True)
...
must = ["app/nova.py", "app/nova_think.py", "app/web_server.py",
        ...
        "app/tests/test_v8120_master_switch.py",
        "app/tests/test_v8110_stability.py",
        "app/tests/test_v8101_stability.py",
        "app/scripts/package_v8120.py",
        "FIXES-8.12.0.fa.md", "PROJECT-MAP.fa.md", "README.md",
...
if PREV.is_file():
    ...
    lost = sorted(f for f in prev if f not in names
                  and f != "FIXES-8.11.0.fa.md")
    probe("no file lost vs v8.11.0 (the old FIXES doc may rotate)",
          not lost, lost[:8])
    probe("fonts intact",
