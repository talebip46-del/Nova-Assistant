#!/usr/bin/env python3
"""Package Nova v8.4.0 "code guardian" - verify then zip (same layout as
the v8.3.0 release: FIXES/*.md at the zip root, app/... under app/).

Release probes (ALL must pass before/after the zip is written):
  1. version pin: nova.VERSION == 8.4.0 (+ nova_think), no stray pins
  2. py_compile every shipped python module
  3. release sanity: the v8.4 code guardian laws (pre-apply deterministic
     gate, repair prompt protocol, local-brain-only model reviews, the
     dedicated "guardian:" websearch namespace, offline KB fallback,
     /guardian command, /api/settings guardian fields, web panel)
     + the v8.3 context engine law + the v8.2.1 portability/hygiene law
  4. full test suite green (re-run INSIDE packaging)
  5. zip content verification (must-have paths incl. the guardian module
     + its tests)
  6. vs v8.3.0: no file lost (nothing intentionally dropped), woff2 intact
...
PREV_CANDIDATES = [
    _OUT_DIR / "nova_assistant_v8.3.0_fixed.zip",
    Path(__file__).resolve().parents[2] / ".." / "download"
    / "nova_assistant_v8.3.0_fixed.zip",
]
...
# files present in v8.3.0 that are INTENTIONALLY gone in this release
...
probe("nova.VERSION == 8.4.0", nova.VERSION == "8.4.0", nova.VERSION)
probe("nova_think.VERSION == 8.4.0", think.VERSION == "8.4.0", think.VERSION)
probe("codename is code guardian", nova.CODENAME == "code guardian",
      nova.CODENAME)
stray = subprocess.run(
    ["grep", "-rln", '"8\\.3\\.0"', str(APP / "tests")],
    capture_output=True, text=True).stdout.strip().splitlines()
probe("no stray 8.3.0 VERSION pins in tests", not stray, stray)
...
print("[probe] v8.4 code guardian laws")
...
probe("guardian module present", (APP / "nova_guardian.py").is_file())
probe("pre-apply fleet gate wired into offer_apply",
...
probe("repair context rides the NEXT user message (one shot)",
      "sess.guardian_repair = None" in src_nova
      and "_grep_ctx = getattr(sess, \"guardian_repair\", None)" in src_nova)
probe("post-apply fleet review feeds last_feedback",
      'last_feedback["guardian"] = g_flat' in src_nova)
probe("feedback note carries guardian findings",
      "GUARDIAN (" in src_nova)
probe("/guardian registered", '"cmd": "/guardian"' in src_nova)
probe("cloud brain never serves the fleet",
      "_coding_brain_is_local" in src_nova
      and "_guardian_chat_fn" in src_nova)
probe("/api/settings guardian fields",
      all(k in (APP / "web_server.py").read_text(encoding="utf-8")
          for k in ("guardian_on", "guardian_model", "guardian_web",
                    "guardian_reset")))
src_ws = (APP / "web" / "index.html").read_text(encoding="utf-8")
probe("web panel ids present",
      all(k in src_ws for k in ("guardianState", "guardianOn",
                                "guardianModel", "guardianWeb",
                                "guardianReject", "guardianSave",
                                "guardianReset", "renderGuardianPanel")))

# behavior spot-checks (offline, real guardian)
os.environ["NOVA_GUARDIAN_WEB"] = "0"
probe("fleet size >= 28 languages", gd.lang_count() >= 28, gd.lang_count())
probe("scanner: string brackets never invent errors",
      gd.scan_delims('x = ")}{"; # }\n', "py")[0] == [])
probe("scanner: heredoc body is free-form",
      gd.scan_delims("cat <<EOF\n} {\nEOF\necho ok\n", "shell")[0] == [])
probe("validator: broken go is caught",
      gd.validate("m.go", "func main( {\n}\n")["errors"] != [])
probe("validator: generic NEVER blocks",
      gd.validate("Dockerfile", "RUN echo ( x\n")["errors"] == [])
probe("validator: markdown NEVER blocks",
      gd.validate("r.md", "see ( [ x\n")["errors"] == [])
probe("validator: binary skipped",
      any("binary" in v for v in
          gd.validate("a.png", "PNG\x00junk\x00")["via"]))
rep = gd.review_batch([("app.py", "def f(:\n")], chat_fn=None,
                      cfg={"model": False, "web": False})
probe("supervisor rejects a broken batch", rep["verdict"] == "reject")
probe("repair prompt speaks the FILE protocol",
      "=== FILE:" in gd.repair_prompt(rep)
      and "NOTHING was written" in gd.repair_prompt(rep))
out = gd.web_lookup("python", lang="python")
probe("offline websearch answers from the KB (never raises)",
      out["ok"] is False and out["source"] == "offline-kb"
      and "print" in out["digest"])
probe("guardian: cache namespace is separate",
      '"guardian:"' in (APP / "nova_guardian.py").read_text(
          encoding="utf-8"))
os.environ.pop("NOVA_GUARDIAN_WEB", None)

# v8.3 law still holds
...
probe("root FIXES-8.4.0 doc present", (ROOT / "FIXES-8.4.0.fa.md").is_file())
probe("README advertises 8.4.0",
      "8.4.0" in (ROOT / "README.md").read_text(encoding="utf-8"))
...
must = ["app/nova.py", "app/nova_guardian.py", "app/nova_ctxengine.py",
        "app/nova_think.py", "app/nova_providers.py", "app/web_server.py",
        "app/web/index.html", "app/tests/test_v840_code_guardian.py",
        "app/tests/test_v830_context_engine.py",
        "app/scripts/package_v840.py",
        "FIXES-8.4.0.fa.md", "PROJECT-MAP.fa.md", "README.md",
...
if PREV.is_file():
    ...
    probe("no file lost vs v8.3.0", not lost, lost[:8])
    probe("fonts intact",
