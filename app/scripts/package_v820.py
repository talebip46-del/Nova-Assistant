#!/usr/bin/env python3
"""Package Nova v8.2.0 - verify then zip (same ROOT-relative layout as
the v8.1.0 release: FIXES/*.md at the zip root, app/... under app/).

Release probes (ALL must pass before/after the zip is written):
  1. version pin: nova.VERSION == 8.2.0 (+ nova_think), no stray pins
  ...
  6. vs v8.1.0: no file lost, woff2 intact
...
PREV = _OUT_DIR / "nova_assistant_v8.1.0_fixed.zip"
...
probe("nova.VERSION == 8.2.0", nova.VERSION == "8.2.0", nova.VERSION)
probe("nova_think.VERSION == 8.2.0", think.VERSION == "8.2.0", think.VERSION)
stray = subprocess.run(
    ["grep", "-rln", '"8\\.1\\.0"', str(APP / "tests")],
    capture_output=True, text=True).stdout.strip().splitlines()
probe("no stray 8.1.0 VERSION pins in tests", not stray, stray)
...
must = ["app/nova.py", "app/nova_think.py", "app/nova_providers.py",
        ...
        "app/tests/test_v810_img_ai.py",
        "app/scripts/live_test_v820_rescue.py",
        "FIXES-8.2.0.fa.md", "README.md", "app/fonts/index.json"]
