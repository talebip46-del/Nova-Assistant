#!/usr/bin/env python3
"""Sync the version pins 8.6.0 -> 8.7.0 across the test files
(the release discipline: ONE version, pinned everywhere).
Also moves the CODENAME assertions to the new "stability sweep"."""
import sys
...
def main():
    ...
    for p in sorted(ROOT.glob("test_*.py")):
        ...
        n = raw.count("8.6.0")
        new = raw.replace("8.6.0", "8.7.0")
        # the codename pins (assertion lines only; historical comments
        # that merely mention the old name stay untouched)
        pin = 'nova.CODENAME, "bug net + vision"'
        n += new.count(pin)
        new = new.replace(pin, 'nova.CODENAME, "stability sweep"')
        if not n:
            continue
