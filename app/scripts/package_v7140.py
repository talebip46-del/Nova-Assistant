#!/usr/bin/env python3
"""Package Nova v7.14.0 - verify then zip.

Release probes (ALL must pass before the zip is written):
  1. version pin: nova.VERSION == 7.13.0 and no stray 7.13.0 pins
  ...
  6. live-test artifact present (scripts/live_test_v7140_coffee.py compiles)
"""
import json
import py_compile
import re
...
def main():
    ...
    strays = [p.name for p in (APP / "tests").glob("test_*.py")
              if "7.13.0" in p.read_text(encoding="utf-8")]
    probe("no stray 7.13.0 pins in tests", not strays, strays)
    ...
    probe("live test script present",
          (APP / "scripts" / "live_test_v7140_coffee.py").is_file())
    ...
    for must in ("app/nova.py", "app/nova_intel.py", "app/web_server.py",
                 "app/web/index.html", "app/tests/test_v7130_intel.py", "app/tests/test_v7140_think.py",
                 "app/nova_think.py", "app/scripts/live_test_v7140_coffee.py",
                 "app/scripts/live_test_v7140_coffee.py", "README.md",
                 "FIXES-7.14.0.fa.md"):
