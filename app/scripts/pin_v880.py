#!/usr/bin/env python3
"""Sync the version pins 8.7.0 -> 8.8.0 across the test files
(the release discipline: ONE version, pinned everywhere).
Also moves the CODENAME assertions to the new "pre-run gate"."""
import sys
...
def main():
    ...
    for p in sorted(ROOT.glob("test_*.py")):
        ...
        n = raw.count("8.7.0")
        new = raw.replace("8.7.0", "8.8.0")
        # the codename pins (assertion lines only; historical comments
        # that merely mention the old name stay untouched)
        pin = 'nova.CODENAME, "stability sweep"'
        n += new.count(pin)
        new = new.replace(pin, 'nova.CODENAME, "pre-run gate"')
        if not n:
            continue
